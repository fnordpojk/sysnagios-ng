...
if [ "$PVERS" != "1" ]; then
	...
	echo "Genereated by Sysnagios<BR>"
...
#Draw 'compressed' graphs if this is for printing
		...
		echo  "<IMG SRC=sysnagios_mkpic.cgi?rrd=//$RRDFILE&timespan=600&width=$WIDTH&height=$HEIGHT&UNIT=Gb ><BR>"

		echo "Statistics for the last hour:<BR>"
		echo  "<IMG SRC=sysnagios_mkpic.cgi?rrd=//$RRDFILE&timespan=3600&width=$WIDTH&height=$HEIGHT&UNIT=Gb ><BR>"

		echo "Statistics for the last 24 hours:<BR>"
		echo  "<IMG SRC=sysnagios_mkpic.cgi?rrd=//$RRDFILE&timespan=86400&width=$WIDTH&height=$HEIGHT&UNIT=Gb ><BR>"
		echo "Statistics for the last week:<BR>"
		echo  "<IMG SRC=sysnagios_mkpic.cgi?rrd=//$RRDFILE&timespan=604800&width=$WIDTH&height=$HEIGHT&UNIT=Gb ><BR>"
		echo "Statistics for the last Month:<BR>"
		echo  "<IMG SRC=sysnagios_mkpic.cgi?rrd=//$RRDFILE&timespan=2851200&width=$WIDTH&height=$HEIGHT&UNIT=Gb ><BR>"
		echo "Statistics for the last Year:<BR>"
		echo  "<IMG SRC=sysnagios_mkpic.cgi?rrd=//$RRDFILE&timespan=31557600&width=$WIDTH&height=$HEIGHT&UNIT=Gb ><BR>"
