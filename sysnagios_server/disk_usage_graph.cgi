...
echo "<HTML><HEAD><TITLE>SYSNAGIOS disk usage graph in % for $SERVICE@$HOST</TITLE>"
...
#Draw 'compressed' graphs if this is for printing
		...
		echo  "<IMG SRC=disk_usage_mkpic.cgi?rrd=//$RRDFILE&timespan=3600&width=$WIDTH&height=$HEIGHT ><BR>"
		echo "<P>"
		echo "Statistics for the last 24 hours:<BR>"
		echo  "<IMG SRC=disk_usage_mkpic.cgi?rrd=//$RRDFILE&timespan=86400&width=$WIDTH&height=$HEIGHT ><BR>"
		echo "<P>"
		echo "Statistics for the last week:<BR>"
		echo  "<IMG SRC=disk_usage_mkpic.cgi?rrd=//$RRDFILE&timespan=604800&width=$WIDTH&height=$HEIGHT ><BR>"
		echo "<P>"
		echo "Statistics for the last Month:<BR>"
		echo  "<IMG SRC=disk_usage_mkpic.cgi?rrd=//$RRDFILE&timespan=2851200&width=$WIDTH&height=$HEIGHT ><BR>"
		echo "<P>"
		echo "Statistics for the last Year:<BR>"
		echo  "<IMG SRC=disk_usage_mkpic.cgi?rrd=//$RRDFILE&timespan=31557600&width=$WIDTH&height=$HEIGHT ><BR>"
		echo "<P>"
echo "</BODY></HTML>"
