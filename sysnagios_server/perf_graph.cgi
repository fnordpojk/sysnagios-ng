...
echo "<HTML><HEAD><TITLE>SYSNAGIOS performance graph for host $HOST</TITLE>"
...
if [ "$PVERS" != "1" ]; then
	...
	echo "Genereated by Sysnagios</A><BR>"
...
#Draw 'compressed' graphs if this is for printing
		echo "Statistics for the last 4 hours:<BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=14400&width=$WIDTH&height=150&image=usage><BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=14400&width=$WIDTH&height=150&image=spec><BR>"

		echo "<P>Statistics for the last 24 hours:<BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=86400&width=$WIDTH&height=150&image=usage><BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=86400&width=$WIDTH&height=150&image=spec><BR>"

		echo "Statistics for the last week:<BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=604800&width=$WIDTH&height=$HEIGHT&image=usage><BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=604800&width=$WIDTH&height=$HEIGHT&image=spec><BR>"

		echo "Statistics for the last Month:<BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=2851200&width=$WIDTH&height=$HEIGHT&image=usage><BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=2851200&width=$WIDTH&height=$HEIGHT&image=spec><BR>"

		echo "Statistics for the last Year:<BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=31557600&width=$WIDTH&height=$HEIGHT&image=usage><BR>"
		echo  "<IMG SRC=perf_mkpic.cgi?rrd=//$RRDFILE&timespan=31557600&width=$WIDTH&height=$HEIGHT&image=spec><BR>"
